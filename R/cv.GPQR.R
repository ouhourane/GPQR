...
function (x, y, group, Kfold = 5,delta=2, taux=0.5, check = c("f1","f2"),gamm = ifelse(method == "GMcp", 3, 4),
  method = c("GLasso","GMcp","GScad","AGMcp","AGScad"), plot.it=FALSE, ...)
  {
    ...
    object<-GPQR(x=x, y=y, group=group,check=check,delta=delta, taux=taux, method=method ,gamm=gamm,...)
    finalfit <- object
    lambda <- object$lambda
    residmat <- matrix(0, length(lambda),Kfold)
    for (i in seq(Kfold)) {
      omit <- all.folds[[i]]
      object <- GPQR(x=x[-omit, ],y=y[-omit],lambda=lambda, group=group,check=check,delta=delta, taux=taux, method=method,gamm=gamm,...)
      nbeta <- rbind(object$b0, object$beta)
      for (j in 1:length(lambda)){
        ypred <- cbind(1, x[omit, ]) %*% nbeta[,j]
        ypred <- y[omit]-ypred
        ypred <- abs(taux - (ypred<0))*abs(ypred)
